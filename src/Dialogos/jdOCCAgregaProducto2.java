...
//-----------------------------------------------------------------------------    
    public Object[] GetFila(){
        ...
        double dif = Double.valueOf(txCantidad.getText()) - Double.valueOf(pfOCCliente.Despacho);
        Object[] obj = null;
        if(Retorno) {
...
//-----------------------------------------------------------------------------    
    private void CargaProductos(String Codigo){
        ...
        ResultSet Rs, Rs2;
        try{
            Rs = Sql.Select("select distinct(p.sku), cc.discontinuado ,p.nombre,u.unidad,u.um, i.stock \n" +
                            "from producto p\n" +
