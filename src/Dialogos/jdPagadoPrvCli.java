...
//            
             ...
             Rs = Sql.Select("select p.id, p.cuenta, p.fechapago as fecha, \n" +
                             "CASE WHEN g.nombre is null THEN '' ELSE g.nombre END as nombre ,\n" +
                             "b.banco,p.nrodocumento,p.monto,p.usuario,\n" +
                             "p.archivo,tipopago\n" +
                             "from cli_pagos p\n" +
                             "left join par_general g on g.tipo='TIPOPAGOCLI' and g.codigo=p.tipopago\n" +
                             "left join par_banco b on b.codigo=p.banco\n" +
                             "where p.cuenta="+cuenta);
                             //"where p.cuenta="+cuenta+" AND ((p.tipopago IN (300,400,500) AND p.id_cartola > 0) OR (p.tipopago NOT IN (300,400,500)  ))");
