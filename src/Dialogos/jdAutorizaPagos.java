...
//                        "left join bco_cuentas bcta on bc.cuenta=bcta.id \n";
            ...
            String query = "Select cpa.id, \n" +
                           ...
                           "case when cpa.tipopago = 10\n" +
                           "	then 'LUVALY'\n" +
                           "    when bcta.cuenta is null then '0'\n" +
                           "    when cpa.tipopago <> 10 then bcta.cuenta \n" +
                           "    else '0'\n" +
                           "end as cuenta,\n" +
                           "case when cpa.tipopago = 10\n" +
                           "	then (select utilizado from ctactecli where nrodocto = cpa.id_cartola and tipdocto = 'FAC')\n" +
                           "    when bc.abono is null then 0\n" +
                           "    else  \n" +
                           "    bc.abono \n" +
                           "end as abono, \n"+
                        "cpa.id_cartola, \n" +
                        ...
                        query = query + "group by b.nombre, bcta.cuenta, bc.abono, cpa.autorizado, cpa.id_cartola, cpa.tipo_autorizacion, cpa.motivo, cpa.id, cpa.tipopago \n" +
        ...
        {
            fmMain.Mensaje("Error al cargar: "+e);
            Logger.getLogger(jdAutorizaPagos.class.getName()).log(Level.SEVERE, null, e);
