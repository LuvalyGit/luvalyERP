 ...
 */
public class pfNCPBusqueda extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        txNroNDP = new javax.swing.JTextField();
        ...
        jXLabel1.setText("NOTA DE DÉBITO PROVEEDOR");
        ...
        jPanel4.setBorder(javax.swing.BorderFactory.createTitledBorder("Nota de Débito"));
        ...
        jPanel4Layout.setVerticalGroup(
            ...
            .addGroup(jPanel4Layout.createSequentialGroup()
                .addGroup(jPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(jLabel14)
                    .addComponent(txNroNDP, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addGroup(jPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(jLabel5)
        ...
        cbTipoDocumento.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "NCP - Nota Crédito Proveedor", "NDP - Nota Débito Proveedor", "FAP - Factura Proveedor", "FAG - Factura de Gastos" }));
        ...
        cbTipo.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Producto", "Descuento", "Regularización" }));
...
//--------------------------------------------------------------------------------
    ...
    private void btIrActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btIrActionPerformed
        ...
        if(!txProveedor.getText().isEmpty() && Tipo==2){
            ...
            if(ADoc.ShowModal("NDP",txRut.getText().trim())==JOptionPane.YES_OPTION){
            ...
            if(jdNumero.GetNumero() != -99 ){
                ...
                try {
                   ...
                   // Grilla.setValueAt(jdNumero.GetNumero(), Grilla.getSelectedRow(),GetCol("Cantidad"));
                    ...
                    double valor = Double.parseDouble(Grilla.getValueAt(Grilla.getSelectedRow(),GetCol("Cantidad")).toString().replaceAll("\\,",""));
                    
                    Grilla.setValueAt(jdNumero.GetPrecio(), Grilla.getSelectedRow(),GetCol("Unitario"));
               
                    double TotLinea = valor * jdNumero.GetPrecio();
    ...
    private void btGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btGuardarActionPerformed
        ...
        if(txNroNDP.getText().isEmpty()) Validacion = Validacion + "Falta Numero de la Nota de Crédito\n";
        ...
        try {
         ...
         //******************************************************************************************************************************************************   
            ...
            Sql.ExeSql("insert into ctacteprv \n" +
                        "(rut,tipdocto,nrodocto,femision,totalexento,totalafecto, comision, totaliva,totalotroimp,totaldocto,electronica,tipdocorigen,nrodocorigen,pagado,estado,tipopago)\n" +
                        "values (\n" +
                        txRut.getText() + ",\n'" +
                        "NDP" + "',\n" +
                        txNroNDP.getText() +",\n'" +
                        ...
                        ",4)");     //Los guarda como NOTA DEBITO por defecto
            // Detalle
            String Recibido="";
            
            for(int i=0; i< Grilla.getRowCount(); i++){
                   
                if(cbTipo.getSelectedIndex()==0)    //Producto
                   ...
                   //----------- SE SUMA LO INGRESADO EN BODEGA NCP ---------------------------
            ...
            if (cbTipo.getSelectedIndex() == 2){            //Regularización
            
                String Query2 = "SELECT * FROM prv_cuentasxpagar \n" +
                                "WHERE rut = " + txRut.getText() + "\n" +
                                "AND tipdocto IN ('NCP') \n"+
                                "AND nrodocto = "+txNroDocumento.getText();

                Rs4 = Sql4.Select(Query2); 
                ...
                if (Sql4.GetRowCount() > 0){
                    ...
                    double monto = Rs4.getDouble("monto");
                    int nrodocto = Integer.parseInt(txNroNDP.getText().toString().trim());
                    Date fechapago = Rs4.getDate("fpago");
                               
                    Sql4.ExeSql("INSERT INTO prv_pagos \n" +/**Pago**/
            ...
            if (cbTipoDocumento.getSelectedIndex() == 0){
                ...
                Sql3.ExeSql("UPDATE ctacteprv SET \n" +
                             ...
                             "and nrodocto="+ txNroDocumento.getText().trim() +"\n" +
            ...
            Sql.Commit();
            fmMain.Mensaje("Nota Débito Guardada");
  //*********************************************************************************************************************************************************************          
            ...
            String Query5 = "SELECT pp.*, cpp.tipdocto, cpp.nrodocto, cpp.rut FROM prv_pagos pp\n" +            //Busca si la NCP tiene un pago asociado
                            "LEFT JOIN prv_cuentasxpagar cpp ON pp.cuenta = cpp.id\n" +
                            "WHERE  pp.nrodocumento = '"+txNroDocumento.getText().trim()+"'\n" +
                            "AND cpp.rut = "+txRut.getText().trim()+" \n" +
                            "AND cpp.tipdocto IN ('FAP')\n" +
                            "AND pp.tipopago = 4";

                
                Rs5 = Sql5.Select(Query5); 
                            
                if (Sql5.GetRowCount() > 0){            //Si tiene un pago asociado 
                               
                    while (Rs5.next()){
                        
                       id = Rs5.getInt("cuenta");
                       rut = Rs5.getString("rut");
                       nfactura = Rs5.getInt("nrodocto"); 
                       montopagos = Rs5.getDouble("monto"); 
                          
                        String Query6 = "SELECT cpp.* FROM prv_cuentasxpagar cpp\n" +
                                        "WHERE  cpp.nrodocto = "+nfactura+"\n" +
                                        "AND cpp.rut = "+rut.trim()+" \n" +
                                        "AND cpp.id = "+id+" \n" +
                                        "AND cpp.tipdocto IN ('FAP')";
                        
                        
                        Rs6 = Sql6.Select(Query6); 
                        ...
                        if (Sql6.GetRowCount() > 0){
                            ...
                            double pagado = Rs6.getDouble("pagado");
                            pagado = pagado - montopagos;
                          
                            
                            Sql7.ExeSql("UPDATE prv_cuentasxpagar SET \n" +             //Se rebaja el monto utilizado de la NCP
                                        "pagado = "+pagado+" \n" +             
                                        "WHERE nrodocto = "+nfactura+"\n" +
                                        "AND rut = "+rut.trim()+" \n" +
                                        "AND id = "+id+" \n" +
                                        "AND tipdocto IN ('FAP')");
                            ...
                            Sql8.ExeSql("DELETE FROM prv_pagos \n" +             //Se elimina el pago realizado con la NCP
                                        "WHERE cuenta = "+id+" \n" +
                                        "AND tipopago IN (4) \n" +
                                        "AND nrodocumento = '"+txNroDocumento.getText().trim()+"'");
                    ...
                    System.out.println("No Hay Pagos Asociados"); 
    ...
    private javax.swing.JTextField txNroNDP;
