...
//                            "and d.orden='"+ Grilla.getValueAt(Grilla.getSelectedRow(), 3).toString().trim() +"' and d.separado > 0");
             ...
             Rs = Sql.Select("select d.sku, p.nombre,u.unidad,d.separado,d.valorunitario," +
                            ...
                            "else p.valultcompra end as costofinal\n" +
                            "," +
                            "case when p.valultcompra = 0 or p.valultcompra is null \n"+
                            "then (d.valorunitario-p.costofinal)*100/case when d.valorunitario = 0 then 1 else d.valorunitario end\n" +
                            "else (d.valorunitario-p.valultcompra)*100/case when d.valorunitario = 0 then 1 else d.valorunitario end \n"+
                            "end as margen\n" +
                            "from occhdet d\n" +
                            "left join producto p on d.sku=p.sku\n" +
                            "left join par_unidad u on u.codigo=p.unidad\n" +
            ...
            GrillaDet.setDefaultRenderer(Object.class, new Elrender()); 
        } catch (Exception e) {
        }finally{
            Sql.Close();
        }
    ...
    private void trae_margen(){
       ...
       String orden ="";
        ...
        try {
            ...
            System.out.println(e.getMessage());
            Logger.getLogger(pfFechaVencimiento.class.getName()).log(Level.SEVERE, null, e);
        }finally{
            Sql.Close();
        }    
